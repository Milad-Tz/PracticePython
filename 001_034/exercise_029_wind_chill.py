...
# Read the air temperature in degrees Celsius
temperature = float(input("Enter the air temperature in degrees Celsius: "))

# Read the wind speed in kilometers per hour
wind_speed = float(input("Enter the wind speed in kilometers per hour: "))

# Check if the conditions for calculating wind chill index are valid
if temperature <= 10 and wind_speed > 4.8:
    # Compute the wind chill index using the given formula
    wind_chill_index = (13.12 + 0.6215 * temperature
                        - 11.37 * (wind_speed ** 0.16)
                        + 0.3965 * temperature * (wind_speed ** 0.16))

    # Display the wind chill index rounded to the nearest integer
